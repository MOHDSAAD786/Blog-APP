...
const CommentBox = ({ selectedBlog }) => {
    ...
    useEffect(() => {
        const getAllCommentsOfBlog = async () => {
            try {
                const res = await axios.get(`http://localhost:5100/api/v1/comment/${selectedBlog._id}/comment/all`)
    ...
    const commentHandler = async () => {
        try {
            const res = await axios.post(`http://localhost:5100/api/v1/comment/${selectedBlog._id}/create`, { content }, {
                headers: {
    ...
    const deleteComment = async (commentId) => {
        try {
            const res = await axios.delete(`http://localhost:5100/api/v1/comment/${commentId}/delete`, {
                withCredentials: true
            })
            if (res.data.success) {
     ...
     const likeCommentHandler = async (commentId) => {
         try {
             const res = await axios.get(
                 `http://localhost:5100/api/v1/comment/${commentId}/like`,
                 {
                     withCredentials: true,
                 }
             );
